...
def arrange_data(database):
    ...
    database['test 8'] = process_test_8(database['test 8'])
    database['test 10'] = process_test_10(database['test 10'])
    database['strapped'] = process_test_strapped(database['strapped'])
    return database


def process_test(data):
    """
    This function handles with usual cases (no y-intercept recorded,
...
def process_test_1(data):
    ...
    for i in range(len(data)):
        ...
        old_name = 'Anti-Dive/Squat Curve Fits (FL/FR/RL/RR) [N/N]'
        new_name = 'Anti-Dive Front/Anti-Lift Rear Curve Fits (FL/FR/RL/RR) [N/N]'
        data[i] = rename_parameter(new_name, old_name, data[i])
...
def process_test_8(data): 
    y_intercept = ['Kinematic Roll Centre Height (FL/FR/RL/RR) [mm/deg]']
    for i in range(len(data)):
        for para in data[i].keys():
            if para != 'vehicle name':
                data = take_useful_data(data, i, para, y_intercept)
...
def process_test_strapped(data):
    y_intercept = ['Force Anti-Dive Angles (FL/FR/RL/RR) [deg/N]']
    for i in range(len(data)):
        # change the name of some parameters
        old_name = 'Longitudinal Wheel Centre Stiffness (FL/FR/RL/RR) [N/mm]'
        ...
        old_name = 'Anti-Dive/Squat Curve Fits (FL/FR/RL/RR) [N/N]'
        new_name = 'Anti-Lift Front/Anti-Squat Rear Curve Fits (FL/FR/RL/RR) [N/N]'
        data[i] = rename_parameter(new_name, old_name, data[i])
